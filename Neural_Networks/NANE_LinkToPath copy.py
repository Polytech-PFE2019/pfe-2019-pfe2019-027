...
class Keras_Graphsage_Layer(keras.layers.Layer):
    ...
    def __init__(self, graphsage_input_dim=3, graphsage_output_dim=3, input_dim=33, output_dim=33):
    ...
    def build(self, input_shapes):
        ...
        self.vars['neigh_weights'] = glorot([self.graphsage_input_dim, self.graphsage_output_dim], name='neigh_weights')
        self.vars['self_weights'] = glorot([self.graphsage_input_dim, self.graphsage_output_dim], name='self_weights')
...
#        self.eye = tf.placeholder(tf.float32, [ self.nbNodeMax , self.nbNodeMax ], 'Eye')
        ...
        self.agg_matrix['rcn_in'] =  self.RGCN_in_neighAggr_matrix
        print(" self.matrix['rcn_in']  ",  self.agg_matrix['rcn_in'] .get_shape())
        self.agg_matrix['rcn_out'] =  self.RGCN_out_neighAggr_matrix
        print(" self.matrix['rcn_out']  ",  self.agg_matrix['rcn_out'] .get_shape())
    ...
    def _build_net(self):   
        ...
        if (FLAGS.state == 0): # link based
            ...
            self.embedding_x_graphsage = self.Graphsage_Layers_LinearOperator(tf.transpose(divid_x_graphsage), self.numWavelengths,
                                                                                  FLAGS.nb_nane_layers,
                                                                                  self.act)
            self.x_graphsage = tf.concat([self.embedding_x_graphsage, tf.transpose(s_d_v)], axis=1)

#        elif (FLAGS.state == 1):
#            link_list = [i for i in range(self.in_size)]
#            divid_x_graphsage = tf.nn.embedding_lookup(tf.transpose(self.x),
#                                                       link_list[:2 * self.nbLinks * self.nbNodeMax])
#            s_d_v = tf.nn.embedding_lookup(tf.transpose(self.x),
#                                           link_list[2 * self.nbLinks * self.nbNodeMax:])
#            self.embedding_x_graphsage = self.Graphsage_Layers_LinearOperator(tf.transpose(divid_x_graphsage),
#                                                                                  2 * self.nbNodeMax,
#                                                                                  FLAGS.nb_nane_layers,
#                                                                                  self.act)
#            self.x_graphsage = tf.concat([self.embedding_x_graphsage, tf.transpose(s_d_v)], axis=1)
            ...
            #num_units = self.nbNodeMax*FLAGS.hl_embedding_size
            #embeddings_in = self.llfc(self.x, self.nbNodeMax*FLAGS.hl_embedding_size, True, 0)
            #self.x_graphsage = self.gatedGraphNN(embeddings_in, FLAGS.hl_embedding_size, FLAGS.hl_embedding_size, FLAGS.nb_nane_layers)
   #         self.x_graphsage = self.gatedGraphNN(self.x, self.in_size//self.nbNodeMax, FLAGS.hl_embedding_size, FLAGS.nb_nane_layers)
            self.x_graphsage = self.Graphsage_Layers_LinearOperator(self.x, self.in_size // self.nbNodeMax, FLAGS.nb_nane_layers, self.act) 
#   
#            self.x_graphsage = self.x
#            for i in range(FLAGS.nb_nane_layers): 
#                self.x_graphsage = self.llfc_adj(self.x_graphsage, self.nbEmbeddings*FLAGS.hl_embedding_size, True, i)
                         
            #self.x_graphsage = self.NANE_layers(self.x, self.in_size // self.nbNodeMax, FLAGS.nb_nane_layers, self.act)   
            #print('x_graphsage.shape  ', self.x_graphsage.shape ) 

        # RNN
        if (FLAGS.RNN_layer) : 
            rnn_cell = tf.nn.rnn_cell.BasicLSTMCell(FLAGS.size_layer)
      #      rnn_cell = tf.nn.rnn_cell.BasicRNNCell(FLAGS.size_layer)
      #      self.rnn_batch_size = FLAGS.batch_size//FLAGS.time_steps
            self.time_steps = FLAGS.time_steps
     #       self.rnn_state_in = rnn_cell.zero_state(self.rnn_batch_size, tf.float32)
       #     self.rnn_input = tf.reshape(layer,[self.rnn_batch_size, self.time_steps, FLAGS.size_layer])
            self.rnn_input = tf.reshape(self.x_graphsage,[-1, self.time_steps, FLAGS.size_layer])
            self.rnn,self.rnn_state = tf.nn.dynamic_rnn(inputs=self.rnn_input, cell=rnn_cell, dtype=tf.float32)
            fc_layer = tf.reshape(self.rnn,shape=[-1,FLAGS.size_layer])
        else:
            fc_layer = self.x_graphsage        

        for i in range(FLAGS.nb_fc_layers-1): 
            fc_layer = self.llfc(fc_layer, FLAGS.size_layer, True, FLAGS.nb_nane_layers + i)

        #print('fc_layer.shape  ', fc_layer.shape ) 

        y = self.llfc(fc_layer, self.out_size,False, FLAGS.nb_fc_layers+FLAGS.nb_nane_layers-1)# final layer
        
        self.variableData['layer'].append(y)
        return y
    
    def gatedGraphNN(self, embeddings_in, embedding_in_size, embedding_out_size, num_hidden_layers):
        #input_size = self.in_size 
        #numSamples = tf.shape(embeddings_in)[0]  
        #print('tf.shape(embeddings_in) ',embeddings_in.get_shape() )
         
        embeddings_in = tf.reshape(embeddings_in,[-1, self.nbEmbeddings, embedding_in_size])
        print('type embeddings_in ', type(embeddings_in))
        print('embeddings_in.shape  ', embeddings_in.shape ) 
        # Batch matrix multiplication
        neigh_aggr_embeddings_in = tf.einsum('ij,sjd->sid', self.topology, embeddings_in)  # out[s,i,d] = sum_j a[i,j] * in[s,j,d]
        print('type neigh_aggr_embeddings_in ', type(neigh_aggr_embeddings_in))
        print('neigh_aggr_embeddings_in.shape  ', neigh_aggr_embeddings_in.shape ) 
        
        self.vars['neigh_aggr_weights'] = glorot([embedding_in_size, embedding_out_size], name='neigh_aggr_weights')
        before_act = tf.einsum('sni,io->sno', neigh_aggr_embeddings_in, self.vars['neigh_aggr_weights'])  # y[s,n,do] = sum_di x[s,n,di] * w[di,do]
        output = self.act(before_act)
        print('type output ', type(output))
        print('output.shape  ', output.shape )
        
        embeddings_out = tf.reshape(output,[-1, self.nbEmbeddings*embedding_out_size])
        print('type embeddings_out ', type(embeddings_out))
        print('embeddings_out.shape  ', embeddings_out.shape )
        
        
#        ####################
#        split_embeddings_in = tf.convert_to_tensor(tf.split(embeddings_in, num_or_size_splits=self.nbEmbeddings, axis=1))
#        print('type split_embeddings_in ', type(split_embeddings_in))
#        print('split_embeddings_in.shape  ', split_embeddings_in.shape )        
#        
#        # Batch matrix multiplication
#        neigh_aggr_embeddings_in = tf.einsum('ij,jsd->isd', self.topology, split_embeddings_in)  # out[i,s,d] = sum_j a[i,j] * in[j,s,d]
#        print('type neigh_aggr_embeddings_in ', type(neigh_aggr_embeddings_in))
#        print('neigh_aggr_embeddings_in.shape  ', neigh_aggr_embeddings_in.shape ) 
#        
#        self.vars['neigh_aggr_weights'] = glorot([embedding_in_size, embedding_out_size], name='neigh_aggr_weights')
#        before_act = tf.einsum('nsi,io->nso', neigh_aggr_embeddings_in, self.vars['neigh_aggr_weights'])  # y[n,s,i] = sum_di x[n,s,di] * w[di,do]
#        output = self.act(before_act)
#        print('type output ', type(output))
#        print('output.shape  ', output.shape ) 
#        

        oper_topology = tf.linalg.LinearOperatorFullMatrix(self.topology)  # aggreg matrix
        oper_eye_in = tf.linalg.LinearOperatorFullMatrix(tf.eye(embedding_in_size))  # aggreg matrix
        oper_eye_out = tf.linalg.LinearOperatorFullMatrix(tf.eye(embedding_out_size))  # aggreg matrix

        agg_matrix_0 = tf.linalg.LinearOperatorKronecker([oper_topology, oper_eye_in]).to_dense()
        agg_matrix = tf.linalg.LinearOperatorKronecker([oper_topology, oper_eye_out]).to_dense()
        
        neigh_aggr_embeddings_in = tf.matmul(embeddings_in, agg_matrix_0)  # 2D matrix: numSamples x (numEmbeddings * input embed size) 
        #neigh_aggr_embeddings_in = tf.reshape(neigh_aggr_embeddings_in,[-1, embedding_in_size]) # 2D matrix: (numSamples*numEmbeddings) x input embed size
        embeddings_in = tf.reshape(embeddings_in,[-1, embedding_in_size]) # 2D matrix: (numSamples*numEmbeddings) x input embed size

        embedding_size = embedding_in_size
        
        print('embedding_in_size ',embedding_in_size )
        print('embedding_out_size ', embedding_out_size )

        for i in range(num_hidden_layers):            
            rnn_input = tf.reshape(neigh_aggr_embeddings_in,[-1, 1, embedding_size]) # (numSamples*numEmbeddings) x numTimeSteps x  embed size
            rnn_state_in = embeddings_in
            rnn_cell = tf.nn.rnn_cell.GRUCell(embedding_out_size) 
                       
            print('rnn_input.shape ',rnn_input.shape )
            print('rnn_state_in.shape ',rnn_state_in.shape )
            out, rnn_state_out = tf.nn.dynamic_rnn(inputs=rnn_input, cell=rnn_cell, initial_state=rnn_state_in, dtype=tf.float32)
            
            embedding_size = embedding_out_size
            #embeddings_in = tf.squeeze(rnn_state_out[:,0,:]) # (numSamples*numEmbeddings) x output embed size
            embeddings_in  = rnn_state_out

            embeddings_out = tf.reshape(rnn_state_out,[-1, self.nbEmbeddings*embedding_size]) # 2D matrix: numSamples x (numEmbeddings * out embed size) 
            neigh_aggr_embeddings_in = tf.matmul(embeddings_out, agg_matrix) # 2D matrix: numSamples x (numEmbeddings * out embed size)           
#            neigh_aggr_embeddings_in = tf.reshape(neigh_aggr_embeddings_in,[-1, embedding_size]) # 2D matrix: (numSamples*numEmbeddings) x out embed size         
 #           neigh_aggr_embeddings_in = tf.zeros_like(embeddings_in)
#            #tf.zeros([numSamples*self.nbEmbeddings, embedding_in_size]) # 2D matrix: (numSamples*numEmbeddings) x input embed size
#            
#            split0, split1, split2 = tf.split(embeddings_in, num_or_size_splits= tf.ones([]), axis=0)
#            
#            for sample in range(numSamples):
#               embeddings_in_thisSample = embeddings_in[sample*self.nbEmbeddings : (sample+1)*self.nbEmbeddings , : ] # 2D matrix: numEmbeddings x input embed size
#               neigh_aggr_embeddings_in[sample*self.nbEmbeddings : (sample+1)*self.nbEmbeddings , : ] = tf.matmul(self.topology, embeddings_in_thisSample)
        return embeddings_out

    def fully_connected(self, num_hidden_layer, prev_layer, num_units, activation_fn, activation_fun, nameLayer):

        """
        Create a fully connectd layer with the given layer as input and the given number of neurons.
        Thanks to this version we can give names in parameters for tensorboard
        """
    ...
    def llfc_adj(self, prev_layer, num_units, isAct, it):
    ...
    def Graphsage_Layers_LinearOperator(self, input_x, embedding_in_size, nb_nane_layers, activation):

        hl_embedding = input_x
        if (nb_nane_layers == 1):
            output_embedding = self.Graphsage_Layer_embedding_LinearOperator(hl_embedding, embedding_in_size,
                                                                                  FLAGS.embedding_size,
                                                                                  activation,0)
            self.output_embedding = output_embedding
        elif (nb_nane_layers == 2):
            hl_embedding = self.Graphsage_Layer_embedding_LinearOperator(hl_embedding, embedding_in_size,
                                                                         FLAGS.hl_embedding_size, activation,0)
            self.variableData['layer'].append(hl_embedding)

            output_embedding =  self.Graphsage_Layer_embedding_LinearOperator(hl_embedding, FLAGS.hl_embedding_size,FLAGS.embedding_size, activation,1)
            self.output_embedding = output_embedding
            ...
            for i in range(nb_nane_layers - 2):
                hl_embedding = self.Graphsage_Layer_embedding_LinearOperator(hl_embedding, FLAGS.hl_embedding_size,
                                                                             FLAGS.hl_embedding_size, activation,i + 1 )
                self.variableData['layer'].append(hl_embedding)

            output_embedding = self.Graphsage_Layer_embedding_LinearOperator(hl_embedding, FLAGS.hl_embedding_size,FLAGS.embedding_size, activation,nb_nane_layers-1)
            self.output_embedding = output_embedding
        #self.output_embedding = tf.reshape(self.output_embedding,[-1, self.nbEmbeddings*FLAGS.embedding_size])
        #print('type embeddings_out ', type(self.output_embedding))
        #print('embeddings_out.shape  ', self.output_embedding.shape )
        
        self.variableData['layer'].append(self.output_embedding)
    ...
    def NANE_layers(self, embeddings_in, embedding_in_size, nb_nane_layers, activation):
        ...
        for i in range(nb_nane_layers):
            if i > 0 : # excepting first nane layer
                embedding_in_size = FLAGS.hl_embedding_size
            if i == nb_nane_layers-1 :  # last layer
                embedding_out_size = FLAGS.embedding_size
            else :  # excepting last nane layer
                embedding_out_size = FLAGS.hl_embedding_size
    ...
    def NANE_layer(self, embeddings_in, embedding_in_size, embedding_out_size, act_fun, it):
        ...
        with tf.name_scope("GSmean_" + str(it)):
            if (FLAGS.aggregator == 'mean'):
                # Batch matrix multiplication: Aggregation
                neigh_aggr_embeddings_in = tf.einsum('ij,sjd->sid', self.agg_matrix['mean'] , embeddings_in)  # out[s,i,d] = sum_j a[i,j] * in[s,j,d]
                #print('type neigh_aggr_embeddings_in ', type(neigh_aggr_embeddings_in))
                #print('neigh_aggr_embeddings_in.shape  ', neigh_aggr_embeddings_in.shape ) 
                
                self.vars['neigh_aggr_weights'] = glorot([embedding_in_size, embedding_out_size], name='neigh_aggr_weights')
                neigh_aggr_embeddings_out = tf.einsum('sni,io->sno', neigh_aggr_embeddings_in, self.vars['neigh_aggr_weights'])  # y[s,n,do] = sum_di x[s,n,di] * w[di,do]

                self.vars['self_weights'] = glorot([embedding_in_size, embedding_out_size], name='self_weights')
                self_embeddings_out = tf.einsum('sni,io->sno', embeddings_in, self.vars['self_weights'])  #y[s,n,do] = sum_di x[s,n,di] * w[di,do]
                
                output = act_fun(self_embeddings_out+neigh_aggr_embeddings_out)
                #print('type output ', type(output))
                #print('output.shape  ', output.shape )
                
            self.variableData['weight'].append(self.vars['neigh_aggr_weights'])
            self.variableData['weight'].append(self.vars['self_weights'])

 
        output = tf.reshape(output,[-1, self.nbEmbeddings*FLAGS.embedding_size])
        return output

    def Graphsage_Layer_embedding_LinearOperator(self, input_vec, graphsage_input_dim, graphsage_output_dim, act_fun,it):
        
        if (FLAGS.batch_normalization):
            with tf.name_scope("BN_" + str(it)):
             input_vec = tf.layers.batch_normalization(input_vec, center=True, scale=True, training=self.is_train)
             self.variableData['batchnorm'].append(input_vec)

        with tf.name_scope("NANE_layer" + str(it)):
            with tf.name_scope("GSmean_" + str(it)):
                if (FLAGS.aggregator == 'mean'):
                    self.vars['neigh_weights'] = glorot([graphsage_input_dim, graphsage_output_dim],
                                                        name='neigh_weights')
                    self.vars['self_weights'] = glorot([graphsage_input_dim, graphsage_output_dim],
                                                       name='self_weights')
                    self.vars['nane_bias'] = zeros([graphsage_output_dim], name='nane_bias')
                        
                    print('type( self.vars[neigh_weights])', type( self.vars['neigh_weights']))

                    from_neighs = kronecker_product(self.agg_matrix['mean'], self.vars['neigh_weights'])
                    from_self = kronecker_product(self.self_matrix, self.vars['self_weights'])
...
#                    operator_22 = tf.linalg.LinearOperatorFullMatrix(self.vars['self_weights'])  # weight variables
#                    
#                    from_neighs = tf.linalg.LinearOperatorKronecker([operator_1, operator_2]).to_dense()                
#                    from_self = tf.linalg.LinearOperatorKronecker([operator_11, operator_22]).to_dense()
#
#                    print('type(from_neighs)', type(from_neighs))
#                    print(from_neighs)
                    
                    print('from_self\n\n\n\n ', from_self.get_shape())
                    print('from_neighs \n\n\n\n ', from_neighs.get_shape())
                    total_weight = tf.add_n([from_self, from_neighs])
                    total_bias  = tf.tile(self.vars['nane_bias'], [self.nbEmbeddings])
                    print("total_bias ",total_bias.get_shape())
                        
            with tf.name_scope("GSgcn_" + str(it)):
                if (FLAGS.aggregator == 'gcn'):
                    self.vars['weights'] = glorot([graphsage_input_dim, graphsage_output_dim],
                                                  name='weights')
                    ...
                    from_neighs_self = tf.linalg.LinearOperatorKronecker([operator_1, operator_2])
                    from_neighs_self = from_neighs_self.to_dense()
                    total_weight = from_neighs_self
    
            with tf.name_scope("GSmeanOutIn" + str(it)):
                if (FLAGS.aggregator == 'mean_out_in'):
                    self.vars['self_weights'] = glorot([graphsage_input_dim, graphsage_output_dim], name='self_weights')
                    self.vars['in_neigh_weights'] = glorot([graphsage_input_dim, graphsage_output_dim], name='in_neigh_weights')
                    self.vars['out_neigh_weights'] = glorot([graphsage_input_dim, graphsage_output_dim],
                    ...
                    from_neighs_in = tf.linalg.LinearOperatorKronecker([operator_1, operator_2])
                    from_self = tf.linalg.LinearOperatorKronecker([operator_11, operator_22])
                    from_neighs_out = tf.linalg.LinearOperatorKronecker([operator_111, operator_222])
    
                    from_neighs_in = from_neighs_in.to_dense()
                    from_self = from_self.to_dense()
                    from_neighs_out = from_neighs_out.to_dense()
    
                    total_weight = tf.add_n([from_neighs_in, from_self, from_neighs_out])
                
                self.variableData['weight'].append(total_weight)

            return act_fun(tf.matmul(input_vec, total_weight)+total_bias)
...
#        Y_test = test_data["y"][-batchSize:]
        ...
        test_loss, test_predictions, test_lastLayer, ll_batchNorm, ll_weight, ll_bias, ll_beforeSofmax, variable = sess.run(
            [NN.loss, NN.y, NN.x_graphsage, NN.LLBatchNormalisation, NN.wll, NN.bll, NN.llRes, NN.variableData],
...
#        Y_valid = valid_data["y"][-batchSize:]
    ...
    def my_kron(x,y):
    # x,y will be  numpy array with the contents of the placeholder below
        return np.kron(x,y)
